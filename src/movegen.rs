...
pub(crate) fn bishop(mpiece: MovingPiece, board: &RotatableBoard, moves: &mut Vec<Translation>) {
    // Diagonals
    {
...
pub(crate) fn rook(mpiece: MovingPiece, board: &RotatableBoard, moves: &mut Vec<Translation>) {
    // Ranks
    {
        let (rank, file) = split_rwc(mpiece.origin);
        let rank_occupancy = bitlanes::slice(rank * 8, select_all(rotate(board, BoardLayout::Rankwise)));
    ...
    {
        let (rank, file) = split_rwc(mpiece.origin);
        let file_occupancy = bitlanes::slice(file * 8, select_all(rotate(board, BoardLayout::Filewise)));
...
pub(crate) fn queen(mpiece: MovingPiece, board: &RotatableBoard, moves: &mut Vec<Translation>) {
    rook(mpiece, board, moves);
    bishop(mpiece, board, moves);
}

fn pattern<const N: usize>(mpiece: MovingPiece, board: &RotatableBoard, moves: &mut Vec<Translation>,
                           pattern: &Pattern<N>) where LaneCount<N>: SupportedLaneCount {
    let destinations = instantiate_pattern(mpiece.origin, pattern);
    let destinations = !select_color(rotate(board, BoardLayout::Rankwise), mpiece.color) & destinations;
...
pub(crate) fn knight(mpiece: MovingPiece, board: &RotatableBoard, moves: &mut Vec<Translation>) {
    pattern(mpiece, board, moves, &KNIGHT_PATTERN);
}


pub(crate) fn king(mpiece: MovingPiece, board: &RotatableBoard, moves: &mut Vec<Translation>) {
    pattern(mpiece, board, moves, &KING_PATTERN);
}

pub(crate) fn pawn_step(mpiece: MovingPiece, board: &RotatableBoard, moves: &mut Vec<Translation>) {
    let direction: isize = match mpiece.color {
        PieceColor::Black => -1, // black walks towards white
        PieceColor::White => 1 // white walks towards black
    };

    let (origin_rank, origin_file) = split_rwc(mpiece.origin);
    let destination_rank: Rank = (origin_rank as isize + direction) as Rank;
    let destination: RankwiseSquareOrdinal = 8 * destination_rank + origin_file;

    let blocked = is_occupied(rotate(board, BoardLayout::Rankwise), destination);
    if blocked { return; }
    Vec::push(moves, Translation::new(mpiece.origin, destination));
}

pub(crate) fn pawn_2step(mpiece: MovingPiece, board: &RotatableBoard, moves: &mut Vec<Translation>) {
    ...
    let birth_rank: usize = match mpiece.color {
        PieceColor::Black => 6,
        PieceColor::White => 1
    };

    let lava_rank: Rank = (birth_rank as isize + direction) as Rank;
    let destination_rank: Rank = (birth_rank as isize + (2 * direction)) as Rank;
    let (origin_rank, origin_file) = split_rwc(mpiece.origin);
    let destination: RankwiseSquareOrdinal = destination_rank * 8 + origin_file;

    if origin_rank != birth_rank { return; }

    let lava_blocked = is_occupied(rw_board, lava_rank * 8 + origin_file);
    if lava_blocked { return; }

    let destination_blocked = is_occupied(rw_board, destination);
    if destination_blocked { return; }

    Vec::push(moves, Translation::new(mpiece.origin, destination))
}

pub(crate) fn pawn_capture(mpiece: MovingPiece, board: &RotatableBoard, moves: &mut Vec<Translation>) {
    let rw_board = rotate(board, BoardLayout::Rankwise);
    let bb: Bitboard = instantiate_pattern(mpiece.origin, lookup_pawn_capture_pattern(mpiece.color));
    let bb: Bitboard = bb & !select_color(rw_board, mpiece.color);
    let bb: Bitboard = bb & select_all(rw_board);
